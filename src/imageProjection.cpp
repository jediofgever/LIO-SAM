...
{
private:
  std::shared_ptr<vox_nav_utilities::GPSWaypointCollector> gps_waypoint_collector_node;

  std::once_flag gps_data_recieved_flag;
  ...
  095 83e2665fe95c0929ef86d679b7f4748ea1262
...
public:
  ...
  : ParamServer("lio_sam_imageProjection", options), deskewFlag(0)
  {
    ...
    gps_waypoint_collector_node = std::make_shared<vox_nav_utilities::GPSWaypointCollector>();
    ...
    {
      std::string map_info_path = std::getenv("HOME") + savePCDDirectory + "../lidar_map_info.yaml";

      if (!gps_waypoint_collector_node->isOrientedGPSDataReady()) {
        RCLCPP_WARN(
          get_logger(),
        ...
        rclcpp::spin_some(gps_waypoint_collector_node);
        rclcpp::sleep_for(std::chrono::seconds(1));
        return;
      } else {
        std::call_once(
          gps_data_recieved_flag, [&map_info_path, this]() {
            RCLCPP_WARN(
              get_logger(),
    ...
    void publishClouds()
    {
      ...
      cloudInfo.cloud_deskewed = publishCloud(
        pubExtractedCloud, extractedCloud, cloudHeader.stamp,
        lidarFrame);
